...
internal sealed class Snes9xRenderer(string corePath, string romsDirectory) : IDisposable
	{
		...
		private readonly string _corePath = corePath;
		private readonly string _romsDirectory = romsDirectory;
    ...
    private static IntPtr _lib;
		...
		private static void FreeNative()
		{
			...
			_unserialize = null!; _getMemoryData = null!; _getMemorySize = null!;
		...
		internal bool Load(Texture2D? targetTexture, string romPath)
		{
			...
			lock (_lock)
			{
				if (string.IsNullOrWhiteSpace(_corePath) || !File.Exists(_corePath))
				{
					AepLog.Error($"[SNES9X] core DLL not found: {_corePath}");
					return false;
				}
            ...
            _sampleRate =
                av.Timing.Sample_rate > 1
                    ? (int)av.Timing.Sample_rate
                    : 48000;
            ...
            AepLog.Debug(
                $"[SNES9X] loaded {Path.GetFileName(romPath)} @ {_fps:0.##}fps, {_sampleRate}Hz");
        }
    ...
    public void Dispose()
        ...
        FreeNative();
			...
			{
				Marshal.FreeHGlobal(_sysDirPtr);
				_sysDirPtr = IntPtr.Zero;
			}
        ...
        nuint pitch)
    {
        ...
        {
            // Never allow broadcast/display failures to kill
            // the libretro emulation thread.
        }
    }
        ...
        nuint frames)
    {
        ...
        {
            // Keep the emulator thread alive if either
            // audio destination has a problem.
        }
        ...
        string publishUrl)
    {
        ...
        {
            AepLog.Warning(
                "[SNES9X] Cannot start game broadcast because SNES is not running.");

            return false;
        }

        if (_broadcastEncoder?.IsRunning == true)
        {
            return true;
        }
        ...
        {
            AepLog.Warning(
                "[SNES9X] Cannot start game broadcast because no SNES video frame has been received yet.");
        ...
        {
            AepLog.Warning(
                "[SNES9X] Cannot start game broadcast because the audio sample rate is invalid.");
                ...
                _sampleRate))
        {
            ...
            AepLog.Error(
                "[SNES9X] Failed to start game broadcast.");
        ...
        AepLog.Info(
            $"[SNES9X] Game broadcast started at {_videoWidth}x{_videoHeight} @ {_fps:0.###}fps, {_sampleRate}Hz.");
    ...
    internal void StopBroadcast()
    {
        ...
        {
            AepLog.Warning(
                $"[SNES9X] Error while stopping game broadcast: {exception.Message}");
        }
...
internal sealed class Snes9xAudio : IDisposable
    ...
    internal float MaskStrength = 0.30f;
    ...
    // Negative maskStrength means CRT processing is disabled.
    // Keep the same scaler/aspect-ratio path, but return the
